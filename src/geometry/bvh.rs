...
use linalg::{Axis, Point, Ray, Vector};
...
impl<T: Boundable> BVH<T> {
    ...
    fn build(
        build_info: &mut [GeomInfo<T>],
        ordered_geom: &mut Vec<usize>,
        ...
        *total_nodes += 1;
        ...
        let bounds = build_info
            .iter()
            .fold(BBox::new(), |b, g| b.box_union(&g.geom.bounds(start, end)));
        ...
        let centroids = build_info
            .iter()
            .fold(BBox::new(), |b, g| b.point_union(&g.center));
        ...
        if ngeom < 5 {
            ...
            if ngeom > max_geom || min_cost < ngeom as f32 {
                mid = partition(build_info.iter_mut(), |g| {
                    let b = ((g.center[split_axis] - centroids.min[split_axis])
                        / (centroids.max[split_axis] - centroids.min[split_axis])
                        * buckets.len() as f32) as usize;
                    let b = if b == buckets.len() { b - 1 } else { b };
